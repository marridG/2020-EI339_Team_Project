...
class BeeHive(object):
    ...
    def __init__(self                 ,
        ...
        # creates a bee hive
        self.population = [ Bee(lower, upper, fun) for i in range(self.size) ] #shape (5, 2)
    ...
    def _mutate(self, dim, current_bee, other_bee):
        ...
        tmp=[]
        for i in range(2):
            tmp.append(self.population[current_bee].vector[dim][i]    + \
               (random.random() - 0.5) * 2                 * \
    ...
    def _check(self, vector, dim=None):
        ...
        for i in range_:
            for j in range(2):
                # checks lower bound
                if  (vector[i][j] < self.lower[i]):
                    vector[i][j] = self.lower[i]

                # checks upper bound
                elif (vector[i][j] > self.upper[i]):
                    vector[i][j] = self.upper[i]
